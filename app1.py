...
def get_geo(ip):
    try:
        ...
        print(f"ipinfo.io failed for {ip}: {str(e)}")
        ...
        try:
            ...
            print(f"ipapi.co failed for {ip}: {str(e2)}")
            return None
...
def AnomalyDetection():
    ...
    st.write("Upload a PCAP file to detect potential anomalies like port scanning or ICMP flood attacks.")
    ...
    # Set thresholds for ICMP flood detection
    icmp_threshold = st.sidebar.slider("ICMP Flood Threshold", 1, 100, 50)
    time_window = st.sidebar.slider("Time Window (seconds)", 1, 10, 1)

    # Start the anomaly detection when the file is uploaded
    if st.button("Start Anomaly Detection"):
        ...
        if uploaded_file is not None:
            ...
            try:
                ...
                st.write(f"Total packets in file: {len(packets)}")
                ...
                for packet in packets:
                    if IP in packet:
                        ...
                        if TCP in packet:
                            ...
                            tcp_connections[src_ip].append((dst_port, time))

                        # Detect ICMP flood
                        if ICMP in packet:
                            icmp_flows[src_ip].append(time)

                # 1. Port Scanning Detection
                st.write("\n[*] Detecting Port Scanning:")
                scanning_results = []
                for ip, connections in tcp_connections.items():
                    unique_ports = set([conn[0] for conn in connections])
                    if len(unique_ports) > 2:  # Threshold for multiple ports being scanned
                        scanning_results.append({
                            "IP": ip,
                            "Unique Ports Scanned": len(unique_ports),
                            "Ports": list(unique_ports)
                        })

                if scanning_results:
                    df_scanning = pd.DataFrame(scanning_results)
                    df_scanning.index = range(1, len(df_scanning) + 1)  # Set index starting from 1
                    df_scanning.index.name = 'S.No'  # Rename the index column to 'S.No'
                    st.table(df_scanning)
                else:
                    st.write("No port scanning detected.")

                # 2. ICMP Flood Detection (DoS Attack)
                st.write("\n[*] Detecting ICMP Flood (DoS):")
                icmp_results = []
                for ip, times in icmp_flows.items():
                    time_deltas = pd.Series(times).diff().dropna()  # Calculate differences between packet timestamps
                    flow_rate = time_deltas[time_deltas <= time_window].count()  # Count flows within the time window

                    if flow_rate > icmp_threshold:
                        # Record the timestamp of the first suspicious ICMP packet
                        first_time = times[0]
                        try:
                            # Ensure the time is a float and then convert it to a readable format
                            attack_start_time = pd.to_datetime(float(first_time), unit='s')
                        except ValueError as e:
                            st.write(f"Error converting time for {ip}: {e}")
                            attack_start_time = "Invalid Time"

                        icmp_results.append({
                            "IP": ip,
                            "ICMP Packet Count": flow_rate,
                            "Attack Start Time": attack_start_time
                        })

                if icmp_results:
                    df_icmp = pd.DataFrame(icmp_results)
                    df_icmp.index = range(1, len(df_icmp) + 1)  # Set index starting from 1
                    df_icmp.index.name = 'S.No'  # Rename the index column to 'S.No'
                    st.table(df_icmp)
                else:
                    st.write("No ICMP flood detected.")
            except Exception as e:
...
# Function to visualize the network graph using Plotly
def visualize_network2(communication_pairs):
    G = nx.Graph()
...
def main(): 
    ...
    # Streamlit UI with option menu and "Recon" tab logic
    ...
    selected = option_menu(
        menu_title=None,
        options=["Upload File", "Raw Data & Filtering", "Analysis", "Geoplots","Anomaly", "Live Sniffing", "IP Analysis", "Recon","Network Communication Graph","Real-Time Network Communication Graph"],
        icons=["upload", "files", "graph-up", "globe", "exclamation","wifi", "shield", "search","",""],
        menu_icon="cast",
        default_index=0,
    ...
    if selected == "Recon":
        st.header("Recon")
        st.subheader("Choose the type of scan you want to perform:")

        # IP address input
        ip_address = st.text_input("Enter the IP address or Hostname to scan:", "117.232.96.199")
        ...
        with col2:
            if st.button("🛡️ Vulnerability Scan"):
                st.write("Starting Vulnerability Scan...")  # Replace with actual functionality
    ...
    if selected=="Real-Time Network Communication Graph":
        ...
        while True:
            if communication_pairs:
                fig = visualize_network2(communication_pairs)
                ...
                *=.......:@...............-@:...............%+..............:@:.......-*
                ...
                with col3:
                    ...
                    with st.expander("Data In/Out Statistics"):
                        sample_data = {'IN': 300, 'OUT': 200}
                        DataInOutStatistics(sample_data)
    ...
    if selected=="Network Communication Graph":
